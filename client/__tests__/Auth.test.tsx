        ...
        ctx.json([
          {
            ...
            created_at: '2021-04-07 07:32:22',
          },
          {
            id: 2,
            title: 'title2',
...
describe('AdminPage Test Cases', () => {
  it('should route to index-page with login successed', async () => {
    const { page } = await getPage({
      route: '/admin-page',
    })
    render(page)
    expect(await screen.findByText('Login')).toBeInTheDocument()
    userEvent.type(screen.getByPlaceholderText('Username'), 'user3')
  ...
  it('should not route to index-page when login is failed', async () => {
    ...
    // 要素が存在しないことをアサーションする場合にはqueryByを使う
  ...
  it('should route to index-page when register+login successeeded', async () => {
    const { page } = await getPage({
      route: '/admin-page',
    })
    render(page)
